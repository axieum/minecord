package me.axieum.mcmod.minecord.api.chat.discord;
...
import me.axieum.mcmod.minecord.api.chat.MinecraftDispatcher;
 ...
 */
public class MessageUpdateCallback extends ListenerAdapter
{
        ...
        MESSAGE_CACHE.computeIfPresent(event.getMessageId(), (id, context) -> {
            ...
            if (!diffs.isEmpty()) {
                ...
                final Map<String, PlaceholderHandler> placeholders = Map.of(
                    ...
                    "diff", markdown(diffs.getFirst().getOldLine())
                );
    ...
    {
        // Update the message cache
        if (!event.getAuthor().isBot() && ChatConfig.hasChannel(event.getChannel().getIdLong())) {
