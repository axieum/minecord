...
public final class PlaceholdersExt
{
    ...
    public static final NodeParser NODE_PARSER = TagParser.createQuickText();
    ...
    {
        return Minecord.getMinecraft().map(PlaceholderContext::of).orElse(null);
    }
    ...
    {
        return context != null
            ? Placeholders.parseText(node, context)
            : Placeholders.parseNodes(node).toText(ParserContext.of(), true);
    }
