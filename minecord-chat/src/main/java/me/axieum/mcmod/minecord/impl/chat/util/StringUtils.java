...
public final class StringUtils
{
    ...
    {
        // Build the string template once otherwise reuse
        if (discordMinecraftST == null) {
            ...
            final Pattern code = Pattern.compile("```(\\w*)\\n(.*?)\\n?```");
            final Pattern code2 = Pattern.compile("```(.*?)```");
            final Pattern code3 = Pattern.compile("`(.*?)`");
            ...
            discordMinecraftST = new StringTemplate()
                ...
                .transform(s -> code3.matcher(s).replaceAll("\u00A77$1u00A7r"));
        }

        // Apply the appropriate string template against the given contents
        final String result = discordMinecraftST.format(contents);

        // Conditionally apply emoji translations and return
        return getConfig().useUnicodeEmojis ? result : EmojiParser.parseToAliases(result);
    }
